 ...
 */
package com.migratebird.integrationtest;
...
import static com.migratebird.config.MigratebirdProperties.*;
import static com.migratebird.integrationtest.MigrateBirdIntegrationTest.TestScript.*;
import static com.migratebird.util.SQLTestUtils.dropTestTables;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.fail;

/**
 * Integration test for the migratebird: verifies the typical usage scenario's in an integrated way: The migratebird is
 * setup using properties, a real database is used and scripts are created on the file system.
 *
*/
public class MigrateBirdIntegrationTest {
    ...
    @Before
    public void init() {
        scriptsLocation = new File("target", "migratebird-integrationtest/scripts");
        initConfiguration();
    ...
    public void errorInRepeatableScript_fixByRemovingScript() {
        ...
        // repeatable script must be removed from the migratebird_scripts table.
        removeScript(REPEATABLE);
        updateDatabase();
    ...
    /**
     * Verifies that, if the migratebird_scripts table doesn't exist yet, and the autoCreateExecutedScriptsInfoTable property is set to true,
    ...
    public void noFromScratchUpdateIfBaseLineRevisionIsSet() {
        try {
            properties.setProperty(PROPERTY_BASELINE_REVISION, "1.2");

            updateIncremental_fromScratchEnabled();
            fail("Expected MigrateBirdException");
    ...
    public void useScriptParameters() throws IOException {
        ...
        properties.put(PROPERTY_SCRIPT_PARAMETER_FILE, propertiesFile.getPath());
    ...
    private void enableFromScratch() {
        properties.put(PROPERTY_FROM_SCRATCH_ENABLED, "true");
    }

    private void disableFromScratch() {
        properties.put(PROPERTY_FROM_SCRATCH_ENABLED, "false");
    }

    private void allowOutOfSequenceExecutionOfPatches() {
        properties.put(MigratebirdProperties.PROPERTY_PATCH_ALLOWOUTOFSEQUENCEEXECUTION, "true");
    }

    private void setIncludedAndExcludedQualifiers(String includedQualifiers, String excludedQualifiers) {
        properties.put(MigratebirdProperties.PROPERTY_EXCLUDED_QUALIFIERS, excludedQualifiers);
        properties.put(MigratebirdProperties.PROPERTY_INCLUDED_QUALIFIERS, includedQualifiers);
    }
    ...
    private void assertInExecutedScripts(TestScript script) {
        MainFactory mainFactory = new MainFactory(config);
    ...
    private boolean updateDatabase() {
        MainFactory mainFactory = new MainFactory(config);
        DbUpdate dbUpdate = mainFactory.createDbUpdate();
        return dbUpdate.updateDatabase(false);
    }

    private void checkScriptUpdates() {
        MainFactory mainFactory = new MainFactory(config);
        DbUpdate dbUpdate = mainFactory.createDbUpdate();
        dbUpdate.updateDatabase(true);
    }
    ...
    private void clearTestDatabase() {
        dropTestTables(defaultDatabase, "migratebird_scripts", BEFORE_INITIAL_TABLE);
    ...
    private void initConfiguration() {
        properties = new ConfigrationLoader().loadDefaultConfiguration();
        properties.put("database.dialect", "hsqldb");
        properties.put("database.driverClassName", "org.hsqldb.jdbcDriver");
        properties.put("database.url", "jdbc:hsqldb:mem:migratebird");
        properties.put("database.userName", "sa");
        properties.put("database.password", "");
        properties.put("database.schemaNames", "PUBLIC");
        properties.put(PROPERTY_AUTO_CREATE_MIGRATEBIRD_SCRIPTS_TABLE, "true");
        properties.put(PROPERTY_SCRIPT_LOCATIONS, scriptsLocation.getAbsolutePath());
        properties.put(PROPERTY_USESCRIPTFILELASTMODIFICATIONDATES, "false");
        properties.put(PROPERTY_FROM_SCRATCH_ENABLED, "false");
        properties.put(PROPERTY_QUALIFIERS, "special,q1,q2");
        
        config = new Config(properties);

        SQLHandler sqlHandler = new DefaultSQLHandler();
        DataSourceFactory dataSourceFactory = new SimpleDataSourceFactory();
        DatabaseConnectionManager databaseConnectionManager = new DefaultDatabaseConnectionManager(config, sqlHandler, dataSourceFactory);
