 ...
 */
package com.migratebird.script;
...
import static com.migratebird.util.TestUtils.qualifiers;
...
public class ScriptFactoryQualifierTest {
	...
	public void initialize() {
		Properties configuration = new ConfigrationLoader()
				.loadDefaultConfiguration();
		scriptIndexRegexp = configuration
				.getProperty(PROPERTY_SCRIPT_INDEX_REGEXP);
		targetDatabaseRegexp = configuration
				.getProperty(PROPERTY_SCRIPT_TARGETDATABASE_REGEXP);
		qualifierRegexp = configuration
				.getProperty(PROPERTY_SCRIPT_QUALIFIER_REGEXP);
	}
	...
	public void singleQualifierName() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"scripts/01_#qualifier_my_script.sql", null, null);
	...
	public void multipleQualifierNames() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"scripts/01_#qualifier_somethingelse_#another_my_script.sql",
				null, null);
	...
	public void leadingQualifierName() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"#qualifier_my_script.sql", null, null);
		assertQualifierNames(script, "qualifier");
	}
	...
	public void qualifierNamesInPath() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"#1_#2_folder/scripts/#3_release/01_#4_my_script.sql", null,
				null);
	...
	public void noQualifierNames() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"scripts/01_@something_text#noqualifer_my_script.sql", null,
				null);
	...
	public void qualifierNotRegistered() {
		ScriptFactory scriptFactory = createScriptFactoryWithRegisteredQualifiers();
		scriptFactory.createScriptWithoutContent("01_#qualifer_my_script.sql",
				null, null);
	}
	...
	public void registeredAsPatchQualifier() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"01_#patch_my_script.sql", null, null);
	...
	public void qualifierCaseInsensitive() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"#QuAlIfIeR1_#QUALIFIER2_script.sql", null, null);
	...
	public void patchCaseInsensitive() {
		...
		Script script = scriptFactory.createScriptWithoutContent(
				"incremental/02_sprint2/03_#PaTcH_addUser.sql", null, null);
		assertQualifierNames(script, "patch");
		assertTrue(script.isPatchScript());
	}
	...
	private void assertQualifierNames(Script script, String... qualifierNames) {
		...
		AssertUtils.assertPropertyLenientEquals("qualifierName",
				asList(qualifierNames), qualifiers);
	}

	private ScriptFactory createScriptFactoryWithRegisteredQualifiers(
			String... qualifierNames) {
		return new ScriptFactory(scriptIndexRegexp, targetDatabaseRegexp,
				qualifierRegexp, qualifiers(qualifierNames),
				new HashSet<Qualifier>(), null, null);
	}

	private ScriptFactory createScriptFactoryWithPatchQualifiers(
			String... qualifierNames) {
		return new ScriptFactory(scriptIndexRegexp, targetDatabaseRegexp,
				qualifierRegexp, new HashSet<Qualifier>(),
				qualifiers(qualifierNames), null, null);
	}
