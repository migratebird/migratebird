...
public class DefaultExecutedScriptInfoSourceRemoveErrorScriptsTest {
	...
	public void failedScripts() throws Exception {
		...
		SortedSet<ExecutedScript> before = executedScriptInfoSource
				.getExecutedScripts();
		AssertUtils.assertPropertyLenientEquals("successful", asList(false, false, false),
				before);

		executedScriptInfoSource.removeErrorScripts();

		SortedSet<ExecutedScript> after = executedScriptInfoSource
				.getExecutedScripts();
	...
	public void successfulScripts() throws Exception {
		...
		SortedSet<ExecutedScript> before = executedScriptInfoSource
				.getExecutedScripts();
		AssertUtils.assertPropertyLenientEquals("successful", asList(true, true, true),
				before);
		...
		SortedSet<ExecutedScript> after = executedScriptInfoSource
				.getExecutedScripts();
		AssertUtils.assertPropertyLenientEquals("successful",
				asList(true, true, true), after);
	}
	...
	private void registerScripts(boolean successful) throws ParseException {
		executedScriptInfoSource.registerExecutedScript(createFailedScript(
				"1_folder/1_script.sql", successful));
		executedScriptInfoSource.registerExecutedScript(createFailedScript(
				"repeatable/script.sql", successful));
		executedScriptInfoSource.registerExecutedScript(createFailedScript(
				"postprocessing/script.sql", successful));
	}

	private ExecutedScript createFailedScript(String scriptName,
			boolean successful) throws ParseException {
		return new ExecutedScript(createScript(scriptName), parseDate(
				"20/05/2008 10:20:00", new String[] { "dd/MM/yyyy hh:mm:ss" }),
				successful);
	}
