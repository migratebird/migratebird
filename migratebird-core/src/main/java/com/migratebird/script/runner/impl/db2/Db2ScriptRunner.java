 ...
 */
package com.migratebird.script.runner.impl.db2;

import static com.migratebird.script.runner.impl.db2.Db2ConnectionInfo.parseFromJdbcUrl;
...
import com.migratebird.script.runner.impl.Application;
import com.migratebird.script.runner.impl.BaseNativeScriptRunner;
...
public class Db2ScriptRunner extends BaseNativeScriptRunner {
    ...
    protected void terminateDb2Connections() {
        try {
            ...
            throw new MigrateBirdException("Unable to terminate database connections.", e);
        }
    }
    ...
    protected void registerDatabase(Db2ConnectionInfo db2ConnectionInfo) {
        try {
            ...
            throw new MigrateBirdException("Unable to register database alias " + db2ConnectionInfo.getDatabaseAlias(), e);
        }
    }
    ...
    protected void unregisterDatabase(Db2ConnectionInfo db2ConnectionInfo) {
        try {
            ...
            throw new MigrateBirdException("Unable to unregister database alias " + db2ConnectionInfo.getDatabaseAlias(), e);
        }
    }
    ...
    protected void executeCommand(boolean logCommand, String... command) {
        ...
        if (exitValue == 4 || exitValue == 8) {
            throw new MigrateBirdException("Failed to execute command. DB2 CLP returned an error.\n" + processOutput.getOutput());
        }
    }
