...
public class Db2ConnectionInfo {
    ...
    protected static Db2ConnectionInfo parseFromType4JdbcUrl(String jdbcUrl, String remoteAlias, String userName, String password) {
        try {
            ...
            if (beginIndex == -1 || endIndex == -1) {
                throw new MigrateBirdException("Invalid url structure.");
            }
            String hostAndPort = jdbcUrl.substring(beginIndex + 2, endIndex);
            if (isBlank(hostAndPort)) {
                throw new MigrateBirdException("Host is missing.");
            }
            String database = jdbcUrl.substring(endIndex + 1);
            if (isBlank(database)) {
                throw new MigrateBirdException("Database is missing.");
            }
            ...
            throw new MigrateBirdException("Unable to parse type 4 DB2 jdbc url. Url should have following form: jdbc:(db2)|(db:net)|(ids)://host(:port)/database. Url: " + jdbcUrl, t);
        }
    }
    ...
    protected static Db2ConnectionInfo parseFromType2JdbcUrl(String jdbcUrl, String userName, String password) {
        try {
            ...
            throw new MigrateBirdException("Unable to parse type 2 DB2 jdbc url. Url should have following form: jdbc:db2:database. Url: " + jdbcUrl, t);
        }
    }
}
