...
public abstract class ScriptContentHandle {
	...
	/*
	 * If true, carriage return chars will be ignored when calculating check
	 * sums
	 */
	...
	/**
	 * @param encoding
	 *            The encoding of the script, not null
	...
	public Reader openScriptContentReader() {
		try {
			scriptReader = new InputStreamReader(getScriptInputStream(),
					encoding);
		} catch (UnsupportedEncodingException e) {
			throw new MigrateBirdException("Unsupported encoding " + encoding, e);
		}
		return scriptReader;
	}
	...
	protected MessageDigest getScriptDigest() {
		...
		try {
			...
			throw new MigrateBirdException(
					"Unable to calculate digest for script.", e);
		} finally {
		...
		try {
			...
			try {
				...
				while ((c = bufferedReader.read()) != -1) {
					...
					if (++count >= maxNrChars) {
						stringWriter
								.write("... <remainder of script is omitted>");
						break;
					}
				}
	...
	public static class UrlScriptContentHandle extends ScriptContentHandle {
		...
		protected InputStream getScriptInputStream() {
			try {
				...
				throw new MigrateBirdException(
						"Error while trying to create reader for url " + url, e);
			}
		}
	}
